...
[ -n "$EVENTBUS" ] || { echo "EVENTBUS env variable must be set to the Event Bus Name ID" ; exit 1; }
...
function send_event ()
{
  [ "$1" = "ready" ] && EVENTFILE="minecraft-server-ready-event.json"
  [ "$1" = "stop" ] && EVENTFILE="minecraft-server-stop-event.json"
  [ "$1" = "willStop" ] && EVENTFILE="minecraft-server-will-stop-event.json"

  echo "Sending $1 event to $EVENTBUS"
  aws events put-events --entries file://$EVENTFILE
}
...
## send event that server is online and ready
cat << EOF >> minecraft-server-ready-event.json
[
	{
		"Source": "minecraft",
		"Detail": "{}",
...
## event that server is stopped
cat << EOF >> minecraft-server-stop-event.json
[
	{
		"Source": "minecraft",
		"Detail": "{}",
...
## event that server will stop soon
cat << EOF >> minecraft-server-will-stop-event.json
[
	{
		"Source": "minecraft",
		"Detail": "{}",
...
do
  ...
  COUNTER=$(($COUNTER + 1))
  [ "$COUTER" == "$SHUTDOWNMIN" ] && send_event willStop
